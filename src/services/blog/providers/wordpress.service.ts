...
const wordpress = axios.create({
  ...
  transformResponse: [
      function (data) {
        const findPath = new URL(process.env.BLOG_PATH).host;
        const replacePath = new URL(process.env.COURSE_URL).host;

        const findProtocol = new URL(process.env.BLOG_PATH).protocol;
        const replaceProtocol = new URL(process.env.COURSE_URL).protocol;
